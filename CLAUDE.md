...
## Token 節約守則（讀檔紀律）
- **>600 行的檔案不要整檔讀**：`training/page.tsx`(1172)、`overview/page.tsx`(1179)、`ExerciseCard.tsx`(1055)、`health/page.tsx`(948)、`training.css`(2356)、`nutrition.css`(1362)、`layout.css`(970)、`copyAllData.ts`(973)、`Shell.tsx`(828)、`LogForms.tsx`、`today.tsx`。先 `grep -n` 定位符號，再用 Read 的 offset/limit 讀該區段。
