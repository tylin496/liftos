...
**It stops on user intent, never on a timer.** Teardown fires on the user's first `wheel`/`touchstart`/`pointerdown`/`keydown`, or when a new nav supersedes it. There's deliberately no `setTimeout` cutoff — if there were, a slow network fetch could finish loading the target card *after* the observer gave up, silently breaking the deep-link. Don't add a timeout here to "clean things up"; the event-based cancellation already covers every real cleanup case (user takes over, or a new nav starts).

**User takeover and a superseding nav tear down DIFFERENT amounts — don't re-merge them.** The arrival highlight (`fireArrival`, scheduled 180ms after the target first shows real content) used to die with the same `cancel()` that stops re-aligning. On a cold entry the cancellable window spans the whole data fetch, so any idle touch while the skeleton loaded silently killed the spotlight — the shipped "first deep-link sometimes has no focus animation" bug (2026-07-21). Now user input calls `stop()` (disconnect observer + listeners, keep the pending highlight — it's a passive cue, not a scroll), while a superseding nav calls `cancel()` (also clears the pending `fireTimer`, so a late fire can't spotlight/dim a page that's no longer on screen). `alignRef` deliberately keeps pointing at `cancel` past a user-input `stop` so a later nav can still reach that pending timer.

**If you're adding a new deep-link target:** you don't need to touch `startAlign` at all — just call `nav(tabId, { scrollTo: "your-element-id" })` (see `NavContext.tsx`) from the source card, and give the target element that `id`. The generic alignment machinery handles the rest, cold or warm. The target does NOT have to be a direct `.page` child: the arrival-spotlight dim selector exempts both the arrived card and any page child that *contains* it (`:not(:has([data-arrived]))` in layout.css) — that `:has` exists because nutrition-today-card lives inside TodayView's drag container, and without it the dim blurred the very card being spotlit (2026-07-21). Don't simplify it away. The arrived marker is a `data-arrived` attribute, not a class — React rewrites the class attribute whenever a card's rendered className changes (any state flag flipping mid-lift), which would strip a classList-added marker and drop the card into its own dim; attributes React didn't render survive. Don't "normalize" it back to a class.
...
**`stopPropagation` alone can still lose an intermittent race on real touch hardware (2026-07-08).** Overview's weight sparkline (`useChartScrub` — press-drag-to-scrub a chart, a DIFFERENT mechanism from `useHorizontalSwipe`, built on Pointer Events + a parallel native-Touch-stopPropagation shim) occasionally still triggered Shell's cross-tab swipe despite that shim, even though a scripted touch-drag repro couldn't reproduce it (confirmed the propagation-based block works in that harness). The plausible cause: WebKit's native gesture recognizer runs on a separate recognition path from JS event dispatch, and can in rare cases (notably a fast/flicked touch) claim a gesture before a JS `stopPropagation()` call has any effect — a timing race, not a logic bug, which is why it's intermittent and hard to repro deterministically. Fix: a SECOND, independent gate that doesn't depend on propagation timing at all — the scrub element tags itself with `data-own-gesture="true"`, and Shell's `onTouchStart` checks the touch's target against that marker directly (`closest('[data-own-gesture="true"]')`); if it matches, Shell never starts tracking axis/dx for that touch's whole lifetime, checked once at touchstart and gated through move/end via a `touchOwnedElsewhere` ref. **If a future gesture-owning element (chart, custom drag, etc.) reports "still sometimes triggers the tab swipe" despite already using `stopPropagation` correctly, don't assume the propagation logic is wrong — add this same marker-based second gate instead of trying to "fix" propagation harder.**

**The marker must be read on BOTH Shell's touch path AND its mouse path — they're separate, unrelated event streams.** The first pass of this fix only wired `data-own-gesture` into `onTouchStart`, and the report came back "fixed on mobile, still happens on desktop." Cause: `useChartScrub`'s `stopPropagation()` runs on the chart's *Pointer* events, but a mouse click ALSO independently dispatches native `mousedown`/`mousemove` — a completely separate event type the browser fires in parallel, not a wrapper around PointerEvent. Stopping propagation on one event type does nothing to a different event type firing for the same physical click, so the chart's own guard never touched Shell's desktop `onMouseDown` (see §"desktop mouse tab-swipe" above — it listens to raw `mousedown`, independently of Pointer/Touch). Any marker-based gate like this needs the SAME check added to both `onTouchStart` and `onMouseDown` from the start — don't assume fixing one event family covers the other.
