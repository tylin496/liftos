...
function handlePhases(res) {
  ...
  for (const entry of state.entries) {
    ...
    if (!phaseMap.has(key)) {
      phaseMap.set(key, {
        ...
        name: entry.cutPhaseName || "Cut Phase",
        index: entry.cutPhaseIndex ?? null,
      });
    } else {
  ...
  sendJson(res, 200, { ok: true, phases });
}

function handleEntries(res, url) {
  const since = url.searchParams.get("since") || formatDate(addDays(new Date(), -29));
  const until = url.searchParams.get("until");
  const filtered = state.entries
    .filter((entry) => entry.date >= since && (!until || entry.date <= until))
    .map(({ id: _id, cutStartDate: _cs, cutPhaseIndex: _ci, cutPhaseName: _cn, cutWeek: _cw, ...rest }) => rest);
  sendJson(res, 200, { ok: true, entries: filtered });
}
