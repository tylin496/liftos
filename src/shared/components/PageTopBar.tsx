...
export function PageTopBar({
  ...
  // Fixed Apple-Fitness "Move" red at every fill, matching the Active Target card
  // ring (no spectrum ramp, no gold flip at 100%); grey when there's no data.
  const ringColor = ringPct == null ? "var(--rule-strong)" : "var(--ring-move)";
  ...
  return (
    <div className="page-topbar">
      ...
      <div className="page-topbar-actions">
        ...
        {onCopy && (
          <span className="copy-all-wrap">
            {/* "Copied" confirmation pill — pops in beside the button on success,
                holds with it, then fades out just before the check reverts. Accent,
                never --good: copy is everyday confirmation, not a status verdict. */}
            <span className={`copy-copied-pill${copied ? " is-shown" : ""}`} aria-hidden="true">
              Copied
            </span>
            <button
              type="button"
